...
void updateSquadDynamics(Career& career, int pointsDelta) {
    ...
    int captainIndex = team_mgmt::playerIndexByName(*career.myTeam, career.myTeam->captain);
    if (captainIndex >= 0 && captainIndex < static_cast<int>(career.myTeam->players.size())) {
        captainLeadership = career.myTeam->players[static_cast<size_t>(captainIndex)].leadership;
    }
    ...
    auto expectedStartsForPromise = [&](const Player& player) {
        ...
        if (player.promisedRole == "Proyecto") {
            return (player.age <= 22) ? max(1, career.currentWeek / 4) : max(0, career.currentWeek / 6);
        }
